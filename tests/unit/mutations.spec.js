...
test('le "setPlayers" affecte un table de joueurs de tennis en "state.players"', ()=>{
    
    mutations.setPlayers(state,playersTennis)
  expect(state.players).toBe(playersTennis)
})
test('le"setShowModal" affecte true ou false pour "state.showModal"',()=>{
  mutations.setShowModal(state,true)
  expect(state.showModal).toBe(true)
})

test('le"setSelectPlayer" affecte objecte d un seul joueur  pour "state.player"',()=>{
  let onePlayer =  {
    "id": 102,
    "firstname": "Serena",
    "lastname": "Williams",
  ...
  mutations.setSelectPlayer(state,onePlayer)
  expect(state.player).toBe(onePlayer)
})